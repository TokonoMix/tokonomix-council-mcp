...
- docs: the README now reports our SWE-bench validation honestly — a cross-family
  council showed no measurable net uplift in bug-catch rate over re-running a single
  agent. We do not claim "more accurate" or "catches more bugs"; the measured value is
  variance-elimination, verification, judge-independence, EU data residency, and
  grounding. No change to tool behaviour.
- improvement: the skill again **proactively offers** a cross-model check before
  irreversible or customer-facing actions (a DB migration, a security-relevant change,
  a legal/compliance or customer-facing statement), instead of only firing when you
  explicitly ask. It offers once and respects your budget; it never claims to guarantee
  correctness. No change to tool behaviour.
- docs: model-catalogue filter descriptions now frame EU routing as **data residency**
  (a hedge against transatlantic data transfer), not a compliance or sovereignty
  guarantee. Wording only — no change to which models are returned.

## [1.7.9] — June 2026

- improvement: when a consensus call is eligible for feedback, the tool output
  now shows a clear "ask your human" section — summarise the result and cost to
  the person you're working with, ask whether the cross-check helped, and relay
  their answer. Previously the tool only prompted your own rating, so the human's
  verdict was never collected even when they were right there.

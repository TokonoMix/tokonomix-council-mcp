...
// Import the exported helpers. Because index.ts is a single-file server with
// a runnable main() at the bottom, we must be careful: the exports are
// declared with `export` so they are importable, but the file also starts the
// MCP server when run directly. The MCP server bootstrap is guarded behind
// main() which we do NOT call here; the import only loads the module-level
// declarations.
import {

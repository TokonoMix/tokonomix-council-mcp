...
import { listModelsSummary } from './list-models-summary.js';
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    if (name === 'tokonomix_list_models') {
      ...
      if (typeof a.max_p95_ms === 'number') params.set('max_p95_ms', String(a.max_p95_ms));
      if (typeof a.speed_tier === 'string') params.set('speed_tier', a.speed_tier);
      ...
      const r = result as {
        data: Array<{
          id: string; owned_by: string; hosting_region: string; origin_country: string | null;
          context_window: number | null; input_per_1m_cents: number | null; output_per_1m_cents: number | null;
          // INT-2570 — per-unit sell pricing (image gen/edit, ASR); optional so
          // an older gateway response still renders.
          per_image_cents?: number | null; per_image_edit_cents?: number | null; per_audio_minute_cents?: number | null;
          capabilities: Record<string, unknown>; tier: string | null;
          // latency-routing ticket 03 — absent unless recent-speed exposure is live.
          speed?: { tier: string; workload_p95_ms: number | null; responsiveness_ms: number | null; note: string };
        }>;
        // INT-2569: post-filter total + applied limit. Optional so an older
        // gateway response still renders.
        total?: number;
        limit?: number;
        truncated?: boolean;
      };
      ...
      const lines = r.data.map((m) => {
        const caps = Object.entries(m.capabilities)
          ...
          .join(',');
        const speed = m.speed
          ? `\tspeed=${m.speed.tier}${m.speed.workload_p95_ms != null ? `(${m.speed.workload_p95_ms}ms)` : ''}`
          : '';
        // INT-2570: per-unit-billed models (image gen/edit, ASR) have no token
        // prices — render their per-unit sell price instead of "null/nullc per 1M".
        let price: string;
        if (m.input_per_1m_cents != null || m.output_per_1m_cents != null) {
          price = `${m.input_per_1m_cents ?? '—'}/${m.output_per_1m_cents ?? '—'}c per 1M`;
        } else if (m.per_image_cents != null || m.per_image_edit_cents != null) {
          const gen = m.per_image_cents != null ? `${m.per_image_cents}c/image` : '';
          const edit = m.per_image_edit_cents != null ? `${m.per_image_edit_cents}c/edit` : '';
          price = [gen, edit].filter(Boolean).join(' ');
        } else if (m.per_audio_minute_cents != null) {
          price = `${m.per_audio_minute_cents}c/audio-min`;
        } else {
          price = 'price unavailable';
        }
        return `${m.id}\t${m.hosting_region ?? '?'}\t${m.origin_country ?? '?'}\t${m.tier ?? '?'}\t${price}\t[${caps}]${speed}`;
      });
    ...
    if (name === 'tokonomix_relay_human_feedback') {
      ...
      try {
        ...
        // Only a `relay_disabled` 404 is reported as dormant; every other 404 gets an
        // honest "not found under this key" (INT-2502). Non-404s rethrow.
        const relayMessage = describeRelayFailure(e);
        if (relayMessage) {
          return {
            content: [
              { type: 'text', text: relayMessage },
              skillVersionTrailer(),
            ],
          };
        }

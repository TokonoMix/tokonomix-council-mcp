...
/**
 * A failed /api/v1 call, carrying the gateway's machine-readable fields alongside
 * the human message.
 *
 * The `message` is byte-identical to what the plain `Error` used to carry, so every
 * existing `msg.includes('Tokonomix API 404')` check keeps working. The added
 * `status`/`code` fields exist so a caller can branch on WHICH error the gateway
 * returned instead of inferring a cause from the status alone — inferring is how
 * the relay ended up reporting "DORMANT" for a request that simply belonged to a
 * different key (INT-2502).
 */
export class TokonomixApiError extends Error {
  readonly status: number;
  /** The gateway's `error.code`, or `'unknown'` when the body carried none. */
  readonly code: string;
  readonly requestId: string | undefined;

  constructor(status: number, code: string, message: string, requestId?: string) {
    super(message);
    this.name = 'TokonomixApiError';
    this.status = status;
    this.code = code;

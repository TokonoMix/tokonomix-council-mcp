...
export const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        context: {
          type: 'object',
          description: 'Optional grounding context (INT-1817). Inline files/snippets are sent to ALL proposers AND the judge so the council reasons over the same source instead of guessing. Large payloads route to tokonomix_upload. Server-gated on the account\'s context-upload capability; when it is not enabled for the account, context is ignored (the prompt-only call is unchanged).',
          properties: {
            inline: {
              type: 'array',
  ...
  },
  {
    ...
    description: [
      ...
      'When recent-speed exposure is live on the platform, entries also carry a `speed` object (tier + recent p95/responsiveness numbers, advisory only — not a guarantee) and you can filter with max_p95_ms / speed_tier to self-select faster models; if the feature is not yet live these two filters are no-ops.',
    ].join(' '),
    inputSchema: {
      type: 'object',
      properties: {
        ...
        tier: {
          type: 'string',
          enum: ['A', 'B', 'C'],
          description: 'Filter to one model tier (catalog quality/pricing tier — not the recent-speed tier, see speed_tier).',
        },
        supports: {
          type: 'array',
          items: {
        ...
        limit: {
          type: 'integer',
          description: 'Max results. Default 500, max 1000. The reply reports how many models matched in total, so you can tell whether the list was cut off.',
        },
        origin_country: {
          type: 'string',
        ...
        max_p95_ms: {
          type: 'integer',
          description: 'Only return models whose recent gateway p95 latency (EWMA) is at or under this ceiling, in milliseconds. Models with no recent gateway data are excluded (can\'t confirm they\'re fast). Advisory, recent-observed — not an SLA. No-op if recent-speed exposure is not yet live.',
        },
        speed_tier: {
          type: 'string',
          enum: ['fast', 'medium', 'slow', 'unknown'],
          description: 'Filter to one recent-speed tier (distinct from `tier` above). "unknown" = too little recent traffic to classify. No-op if recent-speed exposure is not yet live.',
        },
      },
    },
  },
  ...
  },
  {
    ...
    description: [
      ...
      'Tokonomix is in beta: the server requires the user to accept the beta terms (functional gaps, use at your own risk — https://tokonomix.ai/beta) before it will send a code.',
      'If you call this WITHOUT accept_beta_terms and beta acceptance is required, the server replies with a 400 asking you to confirm the beta terms with your human and re-call with accept_beta_terms: true.',
      'So: confirm the beta terms with your human, then call this tool with accept_beta_terms: true.',
      'After a code is sent, instruct the user to check their email and call tokonomix_onboard_verify with the code.',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        accept_beta_terms: {
          type: 'boolean',
          description:
            'Set to true to record the user\'s acceptance of the Tokonomix beta terms (https://tokonomix.ai/beta). REQUIRED while the beta gate is on — only set it true AFTER your human has confirmed they accept. Do not accept on their behalf without confirmation.',
        },
      },
      required: ['email'],
    },
  ...
  },
  {
    ...
    description: [
      ...
      'Feature-gated on the platform feedback-loop: when it is disabled the tool returns a clear "not enabled" message rather than an error.',
    ].join(' '),
    inputSchema: {
      type: 'object',
      properties: {
        ...
        note: {
          type: 'string',
          maxLength: 2000,
          description: 'Optional free-text context (max 2000 chars). Stored admin-only, never returned to any caller and never shown publicly verbatim; enriched by an EU-hosted model into the agent-source day/week/month summary (kept separate from the human-feedback summary). Same privacy handling as the human free text.',
        },
        helped_model: {
          type: 'string',
          maxLength: 200,
  ...
  {
    name: 'tokonomix_relay_human_feedback',
    description: [
      'Relay the HUMAN end-user\'s verdict on a consensus call — the human channel (source_type=\'human_via_agent\'), distinct from tokonomix_rate_consensus (which is the AGENT\'s own rating). Use this ONLY when a real human in the session actually told you what they thought of the council\'s answer — NEVER fabricate or guess a human answer on their behalf; if no human weighed in, do not call this tool.',
      'The `request_id` is returned by tokonomix_consensus_ask in the billing breakdown line (` · request_id: ...`) and in the `x_council.request_id` metadata field.',
      'The `choice` legend (ask the human, then map their answer): 1 = caught a blind spot · 2 = an important improvement · 3 = confirmed my approach · 4 = added nothing · 5 = was wrong/misleading.',
      'Optional: `free_text` accepts up to 2000 chars of the human\'s own words (trimmed and capped server-side); same privacy handling as tokonomix_rate_consensus\'s note — stored admin-only, never returned verbatim to any caller.',
      'Human feedback earns the (higher) human reward rate, versus the agent-source rate on tokonomix_rate_consensus.',
      'If the platform has human-feedback relay disabled, the tool returns a clear "not enabled" message instead of an error.',
    ].join(' '),
    inputSchema: {
      type: 'object',
      properties: {
        request_id: {
          type: 'string',
          description: 'The UUID of the consensus call the human is giving feedback on. Returned by tokonomix_consensus_ask.',
        },
        choice: {
          type: 'integer',
          minimum: 1,
          maximum: 5,
          description: 'The human\'s verdict, 1-5: 1 = caught a blind spot · 2 = an important improvement · 3 = confirmed my approach · 4 = added nothing · 5 = was wrong/misleading.',
        },
        free_text: {
          type: 'string',
          maxLength: 2000,
          description: 'Optional free-text context in the human\'s own words (max 2000 chars).',
        },
      },
  ...
  },
  {
    ...
    description: [
      ...
      'Feature-gated on the account\'s context-upload capability: when it is not enabled the tool returns a clear "not enabled" message rather than an error. Use inline context.inline for small payloads either way.',
    ].join(' '),
    inputSchema: {
      type: 'object',
      properties: {

...
// ─── Skill-source pin (review 4.1 — transparency & control) ──────────────────
// The skill is server-canonical by default so the maintainer can ship skill
// fixes to every remote agent without an npm release. That is a deliberate
// design choice, but it means pinning a package version does NOT pin behaviour.
// Set TOKONOMIX_SKILL_PIN=bundled to opt out: pin the guiding skill to the copy
// that shipped in THIS installed package, make NO skill network call, and refuse
// any server-supplied update-notice — reproducible behaviour for a pinned build.
// Anything else (unset / "live") keeps the default server-canonical behaviour.
// The active source is surfaced on every tool response's trailer (see below).
const SKILL_PIN = (process.env.TOKONOMIX_SKILL_PIN ?? '').trim().toLowerCase();
export function isSkillBundledPinned(): boolean {
  return SKILL_PIN === 'bundled';
}
...
export async function getSkill(full: boolean): Promise<SkillSnapshot> {
  // Pinned to the bundled copy: never touch the network, never accept a
  // server-side update-notice. Reproducible, vendor-independent behaviour.
  if (isSkillBundledPinned()) {
    if (!_skillCache || _skillCache.source !== 'bundled') {
...
/** Where the currently-active skill came from — surfaced on every trailer so
 *  the agent/user can see whether the behaviour-guiding skill is server-fetched
 *  (default) or pinned/bundled (review 4.1 transparency). */
function skillSourceLabel(): string {
  if (isSkillBundledPinned()) return 'bundled(pinned)';
  return (_skillCache?.source ?? 'bundled') === 'server' ? 'server' : 'bundled';
}
...
export function skillVersionTrailer(): { type: 'text'; text: string } {
  ...
  return {
    type: 'text',
    text: `${prefix}\n_skill_version=${cachedSkillVersion()} source=${skillSourceLabel()} (call tokonomix_get_skill if your cached SKILL.md differs${isSkillBundledPinned() ? '; pinned to bundled skill via TOKONOMIX_SKILL_PIN' : ''})`,
  };
}

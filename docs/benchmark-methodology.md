...
## 4c. Every figure here is a dated snapshot (model versions drift)

A house rule that governs the numbers above: **a benchmark figure is a measurement of a specific harness on a specific date, not a permanent property of "the council."** The models named here — the GPT-4o baseline and the cross-family council in §3.1, the panel behind the 5-domain summary in §3.2 — are **versioned, moving targets.** Providers ship new revisions, retire old ones, and change defaults; the same protocol re-run six months later runs against *different* models and can land on a different number. So:

- **Read each figure as "measured on model versions current at the time of that run,"** not as a fixed score you can quote indefinitely.
- **When we publish the raw work** (the harness, the per-case results, the exact model list and the run date — the artifacts §4b says are being prepared), each figure will carry its date and its model versions so it is falsifiable against the same inputs. Until then, the honest framing is the one in §4b: these are our *stated* findings, indicative not conclusive, and the qualitative conclusion (variance-elimination, not "more accurate") is what we stand behind — not any single decimal.

This is the standing rule for any number we put in front of a reader: **falsifiable, and dated to the models it was measured on.**
